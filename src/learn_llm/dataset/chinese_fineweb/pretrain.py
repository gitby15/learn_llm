...
from learn_llm._utils_.pipeline import PackDocumentsNode, Pipeline, PipelineNode, SaveNode
...
def get_train_dataset(tokenizer: AutoTokenizer, context_max_len: int, take_len: int = None) -> Dataset:
    pipeline = Pipeline(
        [
    ...
    print(f"预训练数据总量: {len(dataset):,} 条 × {context_max_len} = {total_tokens:,} tokens ({total_tokens / 1e9:.2f}B)")
    return dataset


if __name__ == "__main__":
    dataset = get_train_dataset(take_len=50)
